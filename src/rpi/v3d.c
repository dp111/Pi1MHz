...
__attribute__((aligned(0x1000))) NOINIT_SECTION static uint8_t v3d_buffer[1024*1048];
...
void v3d_draw_triangle(int x1, int y1, int x2, int y2, int x3, int y3, unsigned int colour) {
  ...
  if (colour == MAGIC_COLOUR) {
     ...
     c1_r = c2_r = c3_r = (float) ((colour >>  6) & 0x3) / 3.0f;
     c1_g = c2_g = c3_g = (float) ((colour >>  2) & 0xF) / 15.0f;
     c1_b = c2_b = c3_b = (float) ((colour      ) & 0x3) / 3.0f;
  } else { // 4 bpp
     c1_r = c2_r = c3_r = (float) ((colour >>  3) & 0x1) / 1.0f;
     c1_g = c2_g = c3_g = (float) ((colour >>  2) & 0x3) / 3.0f;
     c1_b = c2_b = c3_b = (float) ((colour      ) & 0x1) / 1.0f;
  }
...
// Render control list
  ...
  addbyte(&p, 113);
  addword(&p, fb_addr); // framebuffer addresss
  addshort(&p, w); // width
  addshort(&p, h); // height
  if (fb_bpp==32) {
     ...
     addbyte(&p, 0x08); // framebuffer mode ***** TODO ******
  } else if (fb_bpp==4) {
     addbyte(&p, 0x08); // framebuffer mode ***** TODO ******
  }
  addbyte(&p, 0x00);
  ...
  for(x = 0; x < bw; x++) {
    for(y = 0; y < bh; y++) {
      ...
      if (fb_bpp==32) {
         ...
         addbyte(&p, 0x02); // Pixel colour ***** TODO ******
      } else if (fb_bpp==4) {
         addbyte(&p, 0x02); // Pixel colour ***** TODO ******
      }
      addword(&p, fb_addr);
...
int v3d_initialize(uint32_t fb_address, uint32_t width, uint32_t height, uint32_t bpp )
{
