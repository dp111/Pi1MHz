...
uint32_t clmt[SZ_TBL];// upto 63 fragments
...
void filesystemReset(void)
{
   ...
   if (filesystemState.fsMountState == true) {
      ...
      for (uint8_t lunNumber = 0; lunNumber < 8; lunNumber++) {
...
uint32_t filesystemGetLunSizeFromDsc(uint8_t lunDirectory, uint8_t lunNumber)
{
   ...
   if (filesystemState.fsResult == FR_OK) {
      ...
      uint32_t blockSize = (((uint32_t)sectorBuffer[9] << 16) + ((uint32_t)sectorBuffer[10] << 8) + (uint32_t)sectorBuffer[11]);
      uint32_t cylinderCount = (((uint32_t)sectorBuffer[13] << 8) + (uint32_t)sectorBuffer[14]);
      uint32_t dataHeadCount =  (uint32_t)sectorBuffer[15];
