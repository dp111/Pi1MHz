...
void econet_emulator_init(void)
{
   ...
   eco_irq_state   = 0;
   /* Do not write ECO_IRQ_STATUS_REG (0x88) here: helpers_init has
    * already placed the STX opcode at FC88. eco_irq_state starts at 0
    * so eco_irq_update() won't write until the state actually changes. */
