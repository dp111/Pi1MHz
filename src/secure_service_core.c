                           ...
                           uint8_t *command)
{
    command[1] = 1;
    command[2] = 1;
    command[3] = (uint8_t)((service->random_ready ? 1u : 0u) |
                           (service->managed_ssh ? 2u : 0u));
    if (service->managed_ssh && service->port != NULL &&
        service->port->ssh_password != NULL)
        command[3] |= 4u;
    if (service->managed_ssh && service->port != NULL &&
        service->port->sftp_open != NULL)
        command[3] |= 8u;
    command[4] = 0xB8;
    command[5] = 0x88;
                            ...
                            uint8_t *jim, size_t jim_size)
{
    ...
    switch (command[0]) {
    case NTS_SEC_CAPS:
        nts_secure_write_caps(service, command);
        return NTS_OK;
