...
typedef struct nts_secure_service {
    ...
    void *opaque;
    int managed_ssh;
    int random_ready;            /* provider's RNG has actually come up */
} nts_secure_service;

/* The CAPS reply is written from two places - the FIQ fast path, which
   answers it without waiting for the poll loop, and the dispatcher below.
   One implementation, so the two can never describe different capabilities
   to the host. */
void nts_secure_write_caps(const nts_secure_service *service,
                           uint8_t *command);
