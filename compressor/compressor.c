...
int main( int argc , char *argv[]  )
{
    ...
    fclose( fp );
        // input file : Y [7..0] Y[15..8] * 928 * 576
        //             U [7..0] U[15..8] * 928 * 576
        //             V [7..0] V[15..8] * 928 * 576
    // reduce to 8bits per pixel
    // drop to yuv422
    // reorder to yuyv yuyv yuyv yuyv
    for(int i = 0; i < (yuvsize/2/3); i++)
    {
        ydata[i] = ((yuvdatainput[(i*2)+1]<<8)+(yuvdatainput[(i*2)]+128))>>8;
        yuvdata[i*2] =  ydata[i];

    }
    for(int i = 0; i < (yuvsize/2/3); i++)
    {
        int udata1 =  (yuvdatainput[(yuvsize/3) + (i*2) +1 ]<<8) + (yuvdatainput[(yuvsize/3) + (i*2) ]);
        int udata2 = (yuvdatainput[(yuvsize/3) + ((i+1)*2) +1 ]<<8) + (yuvdatainput[(yuvsize/3) + ((i+1)*2) ]);
        udata[i] = (udata1 + udata2 + 256)>>9;
        yuvdata[i*2 + 1] = udata[i];
        i++;
    }
    for(int i = 0; i < (yuvsize/2/3); i++)
    {
        int vdata1 =  (yuvdatainput[(yuvsize/3)*2 + (i*2) +1 ]<<8) + (yuvdatainput[(yuvsize/3)*2 + (i*2) ]);
        int vdata2 = (yuvdatainput[(yuvsize/3)*2 + ((i+1)*2) +1 ]<<8) + (yuvdatainput[(yuvsize/3)*2 + ((i+1)*2) ]);
        vdata[i] = (vdata1 + vdata2 + 256)>>9;
        yuvdata[i*2 + 3] = vdata[i];
        i++;
    }

    writebmp("y.bmp", 928, 576);
