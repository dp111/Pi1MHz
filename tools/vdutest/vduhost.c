...
// same "L .../P x y c" record format the Beeb-side BASIC program emits, so the
...
//   DUMP <x0> <y0> <x1> <y1>   emit "P x y c" for every non-zero pixel in the
//                              box, in pixel coordinates (y up, as POINT
//                              counts), x-major (as the BASIC scan runs);
//                              c is the logical colour, as POINT returns it
...
int main(void)
{
   ...
   while (fgets(line, sizeof line, stdin)) {
      ...
      if (!strncmp(p, "DUMP", 4)) {
         ...
         for (int x = x0; x <= x1; x++)
            for (int y = y0; y <= y1; y++) {
               unsigned int c = (unsigned int)prim_get_pixel(screen, x, y);
               if (c)
                  printf("P %d %d %u\n", x, y, c);
            }
         continue;
