...
run, it would drop the link), `PI_IP` (default `pi1mhz.local`, the name the firmware advertises by mDNS; an address still works), `VIDEO_SET` (VFS set for the VIDEO test; default the first with `video.pvf`), `JUKE` (jukebox set the disc
