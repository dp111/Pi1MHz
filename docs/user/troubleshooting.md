...
Its `/status` page on the web interface does carry a few rows that help
before anything else is tried: **Boot time**, **BREAK** (what happened at
the last BBC reset, and whether the BBC reached the Pi afterwards) and
**Bus diag**. If you are asked for them when reporting a problem, that is
where they are; what each field means is written up for developers in
